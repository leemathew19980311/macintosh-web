        ...
        {defaultConfig.map((item) => {
          if (item.type === ContextMenuType.SEPARATOR) {
            return (
              <div
                key={item.label}
                className="py-0 my-1 mx-2 h-[1px] bg-black/10"
              ></div>
            );
          } else {
            return (
              <ContextMenuItem
                key={item.label}
                className="py-0 px-2 leading-[22px] flex items-center justify-between gap-x-8 hover:!bg-[#0A82FF]/75 hover:!text-white"
              >
                {item.label}
